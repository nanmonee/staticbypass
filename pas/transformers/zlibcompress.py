import os
import string
import random
import zlib

class zlibcompress:

    def __init__(self, arguments: dict) -> None:
        self.name = ''.join(random.SystemRandom().choice(string.ascii_lowercase) for _ in range(16))
        if 'key' in arguments:
            self.key = arguments['key'].encode()
        else:
            self.key = os.urandom(16)

    def imports(self) -> list[str]:
        return ['zstream']

    def compilerOptions(self) -> list[str]:
        return []

    def encode(self, plaintext: bytes) -> bytes:
        self.uncompressedLength = len(plaintext)
        compressed = zlib.compress(plaintext)
        self.compressedLength = len(compressed)
        return compressed

    def transformer(self, shellcodestring: str) -> str:
        return shellcodestring.format(shellcode=f'{self.name}({{shellcode}})')

    def codeblock(self) -> str:
        return f"""
function {self.name}(compressed: TBytes): TBytes;
var
    inputstream: TBytesStream;
    outputstream: TBytesStream;
    zlibstream: TDecompressionStream;
    buffer: array[0..4095] of Byte;
    count: Integer;

begin
    inputstream := TBytesStream.Create(compressed);
    outputstream := TBytesStream.Create;
    zlibstream := TDecompressionStream.Create(inputstream, False);
    try
        repeat
        count := zlibstream.Read(Buffer[0], Length(Buffer));
        if count > 0 then
            outputstream.WriteBuffer(Buffer[0], count);
        until count <= 0;
    finally
        zlibstream.Free;
    end;
    result := outputstream.bytes;
    SetLength(result, outputstream.size);
end;
"""
