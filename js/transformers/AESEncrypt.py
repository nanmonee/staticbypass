...
class AESEncrypt:
    ...
    def transformer(self, shellcodestring: str) -> str:
        return shellcodestring.format(shellcode=f'await {self.name}({{shellcode}})')

    def codeblock(self) -> str:
        return f"""
async function {self.name}(encrypted) {{
    {bytes_to_js(self.key, 'keybytes')}
    {bytes_to_js(self.iv, 'iv')}
